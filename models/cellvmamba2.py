...
class CellVMamba2(nn.Module):
    def __init__(self,
                 ...
                 upsample_depths: List[int] = [2, 2, 2, 2, 2, 2],
                 ssm_d_state: int = 1,
                 ...
                 upsample_drop_path_rate: float = 0.2,
                 ) -> None:
        super().__init__()

        self.dims = dims
        self.drop_rate = drop_rate
        self.upsample_depths = upsample_depths
        self.num_nuclei_classes = num_nuclei_classes
        self.upsample_drop_path_rate = upsample_drop_path_rate
        ...
        self.decoder0 = nn.Sequential(
            Conv2DBlock(in_chans, dims[0], dropout=self.drop_rate)
        )
    ...
    def _forward_upsample(
        ...
        z4: torch.Tensor,
        z5: torch.Tensor,
        branch_decoder: nn.Sequential) -> torch.Tensor:
        ...
        b5 = branch_decoder.decoder4_upsampler(z5) # 32 384 16 16

        b4 = self.decoder4(z4) # 32 384 16 16
        b4 = branch_decoder.decoder3_upsampler(torch.cat([b4, b5], dim=1)) # 32 192 32 32

        b3 = self.decoder3(z3) # 32 192 32 32
        b3 = branch_decoder.decoder2_upsampler(torch.cat([b3, b4], dim=1)) # 32 96 64 64

        b2 = self.decoder2(z2) # 32 96 64 64
        b2 = branch_decoder.decoder1_upsampler(torch.cat([b2, b3], dim=1)) # 32 48 128 128

        b1 = self.decoder1(z1) # 32 48 128 128
        b1 = branch_decoder.decoder0_upsampler(torch.cat([b1, b2], dim=1)) # 32 96 256 256

        b0 = self.decoder0(z0) # 32 96 256 256
        branch_output = branch_decoder.header(torch.cat([b0, b1], dim=1)) # 32 num_classes 256 256
    ...
    def create_upsampling_branch(self, num_classes: int) -> nn.Module:
        ...
        num_layers = len(self.upsample_depths)
        upsample_dims = [self.dims[-1] // 2**i for i in range(num_layers)]
        decoder_dict = OrderedDict()
        dpr = [x.item() for x in torch.linspace(0, self.upsample_drop_path_rate, sum(self.upsample_depths))]

        decoder4_upsampler = nn.Sequential(
              VSSM._make_layer(
              dim = upsample_dims[0], # 768
              drop_path = dpr[:sum(self.upsample_depths[:1])],
              use_checkpoint=False,
              ...
              # =================
              _SS2D=SS2D),
              PatchExpand(dim=upsample_dims[0])) # 768 -> 384; 8x8 -> 16x16

        decoder3_upsampler = nn.Sequential(
              VSSM._make_layer(
              dim = upsample_dims[0], # 384 - 384
              drop_path = dpr[sum(self.upsample_depths[:1]):sum(self.upsample_depths[:2])],
              use_checkpoint=False,
              norm_layer=LayerNorm2d,
              channel_first=True,
              # =================
              ...
              # =================
              _SS2D=SS2D),
              PatchExpand(dim=upsample_dims[0], expand_dim=1)) # 768 -> 192; 16x16 -> 32x32


        decoder2_upsampler = nn.Sequential(
              VSSM._make_layer(
              dim = upsample_dims[1], # 192 - 192
              drop_path = dpr[sum(self.upsample_depths[:2]):sum(self.upsample_depths[:3])],
              use_checkpoint=False,
              norm_layer=LayerNorm2d,
              channel_first=True,
              # =================
              ...
              # =================
              _SS2D=SS2D),
              PatchExpand(dim=upsample_dims[1], expand_dim=1)) # 384 -> 96; 32x32 -> 64x64

        decoder1_upsampler = nn.Sequential(
              VSSM._make_layer(
              dim = upsample_dims[2], # 96 - 96
              drop_path = dpr[sum(self.upsample_depths[:3]):sum(self.upsample_depths[:4])],
              use_checkpoint=False,
              norm_layer=LayerNorm2d,
              channel_first=True,
              # =================
              ...
              # =================
              _SS2D=SS2D),
              PatchExpand(dim=upsample_dims[2], expand_dim=1)) # 192 -> 48; 64x64 -> 128x128
