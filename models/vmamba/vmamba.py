...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.patch_embed = self._make_patch_embed_v2(in_chans, dims[0], patch_size, patch_norm, norm_layer, channel_first=self.channel_first)
    ...
    def load_pretrained(self, f: str):
        weight = torch.load(f, weights_only=True)["model"]
        ...
        for name, value in weight.items():
          ...
          if "downsample" in name:
            ...
            name = f"downsamples.{name_elements[1]}.{name_elements[3]}.{name_elements[-1]}"

          new_weight[name] = value

        message = self.load_state_dict(new_weight, strict=False)
        print(message)
    ...
    def forward(self, x: torch.Tensor):
        ...
        extracted_layers.append(x) # Take the first convolution block
        x = self.patch_embed[4:](x)
...
def build_vssm(yamlfile: str, num_classes: int, in_chans: int, imgsize: int=256) -> VSSM:

  config = yaml.safe_load(open(yamlfile, 'r'))['MODEL']
  vssm = config['VSSM'] 
